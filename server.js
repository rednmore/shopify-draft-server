...
// Petit helper pour masquer les secrets dans les logs
function mask(v){ if(!v) return v; const s=String(v); if(s.length<=8) return '••••'; return s.slice(0,4) + '…' + s.slice(-4); }
...
// Adresse interne pour copie des emails (utilisé ailleurs)
...
app.use(cors({
  origin: function(origin, callback) {
    if (!origin) return callback(null, true); // requests serveur à serveur
...
app.get('/list-customers', async function(req, res) {
  try {
    ...
    if (!shopRes.ok) {
      ...
      return res.status(502).json({ message: 'Shopify non OK', status: shopRes.status, body: txt });
    }
...
// =========================================
// 9. ROUTES D’ASSISTANCE (diagnostic)
// =========================================
app.get('/health', (req, res) => {
  res.json({
    ok: true,
    shop: process.env.SHOPIFY_API_URL || null,
    time: new Date().toISOString()
  });
});

app.get('/', (req, res) => {
  res.type('text/plain').send('Shopify draft server is running.');
});
